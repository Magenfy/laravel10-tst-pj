            ...
            @method('PUT')
                <div class="card-body flex flex-col p-6">
                    <header class="flex mb-5 items-center border-b border-slate-100 dark:border-slate-700 pb-5 -mx-6 px-6">
                        <div class="flex-1">
                            <div class="card-title text-slate-900 dark:text-white">{{ __('Atualizar Produto') }}</div>
                        </div>
                    </header>
                    <div class="card-text h-full space-y-4">
                        <div class="input-area mt-5">
                            <label class="form-label">{{ __('Nome do Produto') }}</label>
                            <input name="name" id="name" type="text" placeholder="Nome do Produto"class="form-control p-[0.565rem] pl-2" value="{{ $product->name }}">
                            <x-input-error :messages="$errors->get('name')" class="mt-2" />
                        </div>

                        <div class="input-area mt-5">
                            <label class="form-label">{{ __('Preço') }}</label>
                            <input name="price" id="price" type="text" placeholder="Preço do Produto"class="form-control p-[0.565rem] pl-2" value="{{ $product->price }}">
                            <x-input-error :messages="$errors->get('price')" class="mt-2" />
                        </div>

                        <div class="input-area mt-5">
                            <label class="form-label">{{ __('Categoria') }}</label>
                            <input name="category" id="category" type="text" placeholder="Categoria"class="form-control p-[0.565rem] pl-2" value="{{ $product->category }}">
                            <x-input-error :messages="$errors->get('category')" class="mt-2" />
                        </div>

                        <div class="input-area mt-5">
                            <label for="largeInput" class="form-label">{{ __('Descrição Curta') }}</label>
                            <textarea name="description" id="description" class="form-control" placeholder="{{ __('Descrição Curta') }}">{{ $product->description }}</textarea>
                    ...
                    <div class="card-body flex flex-col p-6">
                        ...
                        <div class="card-text h-full space-y-4">
                            <div class="input-area">
								...
								<input name="image_url" id="image_url" type="text" placeholder="URL Key Image"class="form-control p-[0.565rem] pl-2" value="{{ $product->image_url }}">
